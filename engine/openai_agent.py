#!/usr/bin/env python3
"""
LawPro Fast Converter - OpenAI GPT Agent
==========================================
...
try:
    ...
    print(json.dumps({
        "type": "error",
        "msg": "openai 패키지가 설치되어 있지 않습니다. pip install openai"
    }), flush=True)
    sys.exit(1)
...
class OpenAIReviewAgent:
    ...
    # 토큰 제한
    MAX_TOKENS = 128000  # GPT-4o 기준
    CHUNK_SIZE = 30000   # 문자 단위 청크 크기

    def __init__(self, api_key: str, model_name: str = "gpt-4o"):
        self.api_key = api_key
        self.model_id = self.MODELS.get(model_name, self.MODELS["gpt-4o"])
    ...
    def _review_chunked(self, content: str) -> str:
        ...
        for i, chunk in enumerate(chunks):
            ...
            reviewed_chunk = self._call_openai(chunk)
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"""아래 HTML 문서에서 OCR 오류를 찾아 수정하고, 수정된 HTML을 반환하세요.
...
{content}"""}
            ...
            self._emit_error(f"OpenAI API 오류: {str(e)}")
            return content
...
# 배치 처리
# ============================================================
def batch_review(folder_path: str, api_key: str, model_name: str = "gpt-4o"):
    """
    폴더 내 모든 문서 배치 검수
    ...
    output_dir = os.path.join(folder_path, "Final_Reviewed_OpenAI")
    ...
    # 에이전트 초기화
    agent = OpenAIReviewAgent(api_key=api_key, model_name=model_name)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(json.dumps({
            "type": "error",
            "msg": "사용법: python openai_agent.py <폴더경로> <API키> [모델명]"
        }), flush=True)
        sys.exit(1)

    folder_path = sys.argv[1]
    api_key = sys.argv[2]
    model_name = sys.argv[3] if len(sys.argv) > 3 else "gpt-4o"
